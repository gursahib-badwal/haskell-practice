...
empty = (pushing_list, popping_list)
        where pushing_list = []
              popping_list = []
...
enqueue ([], popping_list) num = ([num], popping_list)
enqueue (pushing_list, popping_list) num = (num:pushing_list, popping_list)
...
dequeue ([],[]) = (0,empty)
dequeue (pushing_list , h:t) = (h, (pushing_list, t))
dequeue (pushing_list, []) = dequeue([], new_popping_list)
                             where new_popping_list = reverse pushing_list
